...
public class UpdateStaffForm extends JPanel {
    ...
    private void updateStaff() {
        if (staffIdDropdown.getSelectedItem() == null) { setStatus("Please select a staff ID.", true); return; }

        int staffId = Integer.parseInt(staffIdDropdown.getSelectedItem().toString().split(" - ")[0]);
        ...
        if (branchItem == null) { setStatus("Select branch.", true); return; }
    ...
    private void fetchAndPopulate() {
        ...
        if (st == null) { setStatus("Staff not found.", true); return; }
        ...
        try {
            ...
            if (current != null)
                for (int i = 0; i < branchDropdown.getItemCount(); i++)
                    if (branchDropdown.getItemAt(i).startsWith(current + " "))
                        branchDropdown.setSelectedIndex(i);
        } catch (RuntimeException ex) {
            setStatus("Failed to load branches.", true);
        }
    }

    private void loadDays()  { for (int i = 1; i <= 31; i++) dayDropdown.addItem(String.format("%02d", i)); }
    private void loadMonths(){ for (int i = 1; i <= 12; i++) monthDropdown.addItem(String.format("%02d", i)); }
    ...
    private void styleFlatButton(JButton b, Color col) {
        ...
        b.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent e){ b.setBackground(col.darker()); }
    ...
    private static class RoundedShadowPanel extends JPanel {
        ...
        RoundedShadowPanel(int r){ radius = r; setOpaque(false); setLayout(new GridBagLayout()); }
